...
def import_questions(request):
    ...
    if request.method == 'POST':
        # try:
        questions_to_import = request.POST['questions']
        rows = [x.strip() for x in questions_to_import.split("\n") if x.strip()]
        ...
        def create_choice(row, mark):
            if Quest.started:
                Quest.started = False
                Quest.list[Quest.number].save()
            Choice.objects.create(question=Quest.list[Quest.number],
                                  text=row[1:].strip(),
                                  mark=mark)
        for row in rows:
            if row.startswith('-'):
            ...
            elif not Quest.started:
                Quest.number += 1
                Quest.started = True
                Quest.list.append( Question(text=row,
                                               category=category) )
            else:
                Quest.list[Quest.number].text += "\n"+row
        success = True
        # except:
